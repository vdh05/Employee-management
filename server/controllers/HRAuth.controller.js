...
export const HandleHRLogin = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ success: true, message: "HR Login Successfull", type: "HRLogin", token: token, data: HR })
    }
    catch (error) {
...
export const HandleHRForgotPassword = async (req, res) => {
    ...
    try {
        if (!email) {
            return res.status(400).json({ success: false, message: "Email is required", type: "HRforgotpassword" })
        }
